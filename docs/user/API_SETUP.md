...
FLUX에서는 **대시보드의 `AI API 설정` 버튼**으로 연결하는 방법을 권장합니다. API 키를 저장한 뒤 **모델 선택**을 눌러, 해당 키로 사용할 수 있는 모델 목록에서 고릅니다. 기본 주소는 자동으로 적용되며, 일반 사설 API처럼 주소가 다른 경우에만 고급 설정에서 바꿉니다. 모델에 토큰 한도·계정 크레딧 정보가 제공되면 목록에서 마우스를 올려 확인할 수 있습니다.
...
설정 화면에서 **Google AI · Gemini**를 고르고 Google AI Studio에서 발급한 API 키를 입력합니다.

| 항목 | 값 |
| --- | --- |
| 서버 주소 | `https://generativelanguage.googleapis.com/v1beta` |
| 모델 예시 | `gemini-2.5-flash` |
| API 키 | Google AI Studio에서 만든 Gemini API 키 |

FLUX는 Gemini의 `streamGenerateContent` API를 사용해 응답을 실시간으로 표시합니다. 연결 시험은 선택한 Gemini 모델을 직접 조회합니다. API 키는 `x-goog-api-key` 요청 헤더로만 전송됩니다. [Gemini API 공식 참고 문서](https://ai.google.dev/api), [스트리밍 생성 문서](https://ai.google.dev/gemini-api/docs/generate-content/get-started)
...
## 6. 사설 API (일반) — Chat Completions 호환 서버
