...
**이름: FLUX** — Windows와 Linux에서 실행되며, 웹 대시보드와 Discord로 조작하는 개인용·로컬 우선 AI 오케스트레이터.
...
### P1 — Discord·외부 연동·기억·자동화·브라우저

- Discord 단일 봇 채널, 허용 발신자 목록, 봇 메시지 루프 차단
- 사용자가 승인한 유용한 외부 서비스 연결(각 연결의 OAuth/API 키·권한·감사 로그 분리)
...
- 가능한 경우 즉시 삭제 대신 프로젝트별 `.flux-trash/`로 이동하고, 복원 기한을 둔다.
...
- Windows와 Linux에서 `flux start`로 Gateway와 대시보드가 실행된다.
